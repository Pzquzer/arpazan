      ...
      <div ref={ref} className="h-full p-8 md:p-12">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {principles.map((item, index) => (
            <motion.div
              key={item.principle}
              className="bento-card border border-zinc-subtle/15 p-6 min-h-[200px] hover:border-deep-black transition-all"
              initial={{ opacity: 0, y: 40 }}
              animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 40 }}
              transition={{ duration: 0.8, delay: 0.2 + index * 0.1 }}
            >
              <div className="mb-4">
                <h3 className="text-lg md:text-xl font-medium text-deep-black mb-2">
                  {item.principle}
                </h3>
                <p className="text-zinc-subtle font-mono text-xs md:text-sm">
                  → {item.tech}
                </p>
              </div>

              <p className="body-text text-xs md:text-sm text-zinc-subtle leading-relaxed">
                {item.description}
              </p>
            </motion.div>
          ))}
